    ...
    <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      ...
      <div className="grid items-start xl:grid-cols-[380px_minmax(0,1fr)]">
        <form className="space-y-4 border-b border-slate-200 p-5 xl:border-b-0 xl:border-r" onSubmit={submitRequest}>
          ...
          <label className="block"><span className="text-xs font-bold text-slate-600">Reason</span><textarea className="mt-2 min-h-24 w-full resize-y rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-100" maxLength="2000" minLength="10" onChange={(event) => setForm((current) => ({ ...current, reason: event.target.value }))} required value={form.reason} /></label>
          <button className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-emerald-700 px-4 text-sm font-bold text-white hover:bg-emerald-800 disabled:opacity-50" disabled={busy} type="submit"><FiClock />{busy ? "Submitting..." : "Submit correction"}</button>
        </form>
