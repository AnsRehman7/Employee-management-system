        ...
        <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_400px]">
            ...
            <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-200 px-5 py-4">
                <div className="flex items-center gap-3">
                  <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-100 text-emerald-800"><FiBriefcase className="h-4 w-4" /></span>
              ...
              <div className="space-y-5 p-5">
                ...
                {fieldVisible("objective") && <label className="block"><span className="flex items-center gap-2 text-sm font-bold text-slate-700"><FiTarget className="h-4 w-4 text-emerald-600" />Primary objective</span><textarea className="mt-2 min-h-28 w-full resize-y rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-3 text-sm leading-6 text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-100" maxLength="5000" name="objective" onChange={handleChange} placeholder="State the measurable business outcome this project must deliver." required={fieldRequired("objective")} value={formData.objective} /></label>}
                {fieldVisible("description") && <label className="block"><span className="text-sm font-bold text-slate-700">Scope and requirements</span><textarea className="mt-2 min-h-64 w-full resize-y rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-3 text-sm leading-6 text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-100" minLength={formData.generateTasksWithAi ? 40 : undefined} name="description" onChange={handleChange} placeholder="Document deliverables, workflows, constraints, integrations, stakeholders, and acceptance expectations." required={formData.generateTasksWithAi || fieldRequired("description")} value={formData.description} /></label>}
                {formData.generateTasksWithAi && <label className="block"><span className="text-sm font-bold text-slate-700">Atomic requirements</span><textarea className="mt-2 min-h-40 w-full resize-y rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-3 text-sm leading-6 text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-100" name="requirementsText" onChange={handleChange} placeholder={"Users can reset their password by email.\nManagers can approve attendance corrections.\nReports can be exported as CSV."} value={formData.requirementsText} /><span className="mt-1.5 block text-xs text-slate-400">Enter one testable requirement per line. The planner will trace each requirement to delivery tasks.</span></label>}
          ...
          <section className="rounded-lg border border-slate-200 bg-white shadow-sm xl:sticky xl:top-28">
            ...
            <div className="space-y-5 p-5">
              ...
              {fieldVisible("description") && fieldVisible("dueDate") && <label className={`flex cursor-pointer items-start gap-3 rounded-lg border p-4 transition ${formData.generateTasksWithAi ? "border-emerald-300 bg-emerald-50 ring-2 ring-emerald-100" : "border-slate-200 bg-slate-50 hover:border-emerald-200"}`}>
                <input checked={formData.generateTasksWithAi} className="mt-1 h-4 w-4 shrink-0 accent-emerald-700" name="generateTasksWithAi" onChange={handleChange} type="checkbox" />
                <span className="min-w-0"><span className="flex items-center gap-2 text-sm font-bold text-slate-900"><FiCpu className="h-4 w-4 text-emerald-700" />Generate a reviewable delivery plan</span><span className="mt-1 block text-xs leading-5 text-slate-500">Builds traceable tasks, dependencies, milestones, risks, and capacity-aware recommendations. Tasks are created only after management approval.</span></span>
              </label>}
