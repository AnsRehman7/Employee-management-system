              ...
              <div className="p-5 sm:p-6">
                <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                  <div className="min-w-0 max-w-4xl">
                    <div className="flex flex-wrap items-center gap-2"><span className={`rounded-full border px-2.5 py-1 text-xs font-bold ${PROJECT_STATUS_STYLES[project.status] || PROJECT_STATUS_STYLES.active}`}>{labelForValue(project.status)}</span><span className={`text-xs font-bold ${PROJECT_HEALTH_STYLES[project.health] || "text-slate-600"}`}>{labelForValue(project.health)}</span><span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-bold text-slate-600">{labelForValue(project.priority)} priority</span>{project.code && <span className="rounded-full border border-cyan-200 bg-cyan-50 px-2.5 py-1 text-xs font-bold text-cyan-700">{project.code}</span>}</div>
            ...
            <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_330px]">
              ...
              <aside className="rounded-lg border border-slate-200 bg-white shadow-sm">
                ...
                <dl className="divide-y divide-slate-100 px-5">
                  <div className="py-4"><dt className="flex items-center gap-2 text-xs font-bold text-slate-500"><FiUser className="h-4 w-4" />Owner</dt><dd className="mt-2 flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-cyan-100 text-[10px] font-bold text-cyan-800">{initialsFor(project.ownerName)}</span><span className="text-sm font-bold text-slate-900">{project.ownerName}</span></dd></div>
