...
const fieldClass =
  "h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-800 outline-none transition focus:border-violet-400 focus:ring-4 focus:ring-violet-100";
    ...
    <AppShell title="Planning studio" subtitle={project ? `${project.name} / explainable delivery planning` : "Project planning"}>
      <div className="space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-5">
          <Link className="inline-flex items-center gap-2 text-sm font-bold text-slate-600 hover:text-violet-700" to={`/projects/${projectId}`}>
            <FiArrowLeft /> Project
          </Link>
          ...
          <section className="border-y border-slate-200 bg-white py-5">
            <div className="flex flex-wrap items-center justify-between gap-3 px-1">
              ...
              <div className="flex flex-wrap items-end gap-2">
                ...
                <button className="inline-flex h-10 items-center gap-2 rounded-lg bg-violet-600 px-4 text-sm font-bold text-white shadow-sm hover:bg-violet-700 disabled:opacity-60" disabled={Boolean(busy)} onClick={generate} type="button"><FiCpu /> {busy === "generate" ? "Generating..." : "Generate plan"}</button>
              </div>
            </div>
        ...
        {!plan ? (
          <div className="border-y border-slate-200 bg-white py-16 text-center"><FiCpu className="mx-auto h-7 w-7 text-violet-500" /><h2 className="mt-3 text-lg font-bold text-slate-950">No delivery plan yet</h2></div>
        ) : (
          <>
            <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
              <div className="flex flex-col gap-4 border-b border-slate-200 p-5 lg:flex-row lg:items-start lg:justify-between">
                <div className="max-w-4xl"><div className="flex items-center gap-2"><span className="rounded border border-violet-200 bg-violet-50 px-2 py-1 text-xs font-bold text-violet-700">PLAN V{plan.version}</span><span className="text-xs font-bold uppercase text-slate-500">{labelize(plan.status)} / {plan.model}</span></div><p className="mt-3 text-sm leading-6 text-slate-700">{plan.summary}</p></div>
                {plan.status === "draft" && canManage && <div className="flex shrink-0 flex-wrap items-end gap-2"><button className="inline-flex h-10 items-center gap-2 rounded-lg border border-slate-200 px-3 text-sm font-bold text-slate-700 hover:bg-slate-50" onClick={applyRecommendations} type="button"><FiUserCheck /> Apply recommendations</button><button className="inline-flex h-10 items-center gap-2 rounded-lg border border-rose-200 px-3 text-sm font-bold text-rose-700 hover:bg-rose-50" disabled={Boolean(busy)} onClick={reject} type="button"><FiX /> Reject</button><button className="inline-flex h-10 items-center gap-2 rounded-lg bg-violet-600 px-4 text-sm font-bold text-white hover:bg-violet-700" disabled={Boolean(busy)} onClick={approve} type="button"><FiCheck /> Approve plan</button></div>}
