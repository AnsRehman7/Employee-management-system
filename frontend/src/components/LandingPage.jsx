...
const LandingPage = () => {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f4f5fb] text-slate-950">
      ...
      <section className="mx-auto grid max-w-7xl items-center gap-12 px-6 pb-16 pt-10 lg:grid-cols-[1.05fr_0.95fr] lg:pb-24 lg:pt-16">
        <div className="animate-fade-in">
          <p className="mb-5 inline-flex rounded-full border border-violet-200 bg-white px-4 py-2 text-sm font-bold text-violet-700 shadow-sm">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <button
              onClick={() => navigate("/pricing")}
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-violet-600 px-6 py-3 text-sm font-bold text-white shadow-xl shadow-violet-200 transition hover:bg-violet-700"
            >
              View pricing
            ...
            <button
              onClick={() => navigate("/login")}
              className="inline-flex items-center justify-center rounded-lg border border-slate-200 bg-white px-6 py-3 text-sm font-bold text-slate-700 shadow-sm transition hover:border-violet-200 hover:text-violet-700"
            >
              Sign in
            </button>
          </div>
        ...
        <div className="relative animate-fade-in-slow">
          <div className="rounded-lg border border-slate-200 bg-white p-4 text-slate-950 shadow-2xl shadow-slate-200/80">
            <div className="rounded-lg bg-[#f8f8ff] p-5">
              <div className="mb-5 flex items-center justify-between">
                <div>
                  <p className="text-xs font-bold uppercase text-violet-600">Executive dashboard</p>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                {[
                  ...
                  <div key={label} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
                    <span className="mb-3 flex h-9 w-9 items-center justify-center rounded-lg bg-violet-50 text-violet-700">
                      {icon}
                    </span>
                ...
                <div className="flex h-28 items-end gap-2">
                  {[32, 42, 38, 56, 51, 70, 78].map((height, index) => (
                    <div className="flex flex-1 flex-col justify-end" key={index}>
                      <div className="rounded-t-md bg-violet-500/80" style={{ height: `${height}%` }} />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-4 grid gap-3">
                {[
                  ["Campus ERP modernization", "Project workspace", "78% progress", "violet"],
                  ["Attendance export validation", "Operations report", "Review", "cyan"],
